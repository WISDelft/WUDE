...
public class StreamingParser extends Thread {
			...
			public void onStatus(Status status) {
				...
				if (tasks != null) {
					for (Integer taskId : tasks) {
						...
						datalayer.storeTweets(Collections.singleton(tweet), taskId);
					}
	...
	private void refreshFollowers() throws TwitterException {
		...
		while (follow.length > 0 && stream != null) {
			...
			// get new stream
			stream = datalayer.getTwitterStream(++index);
		}
		if (follow.length > 0) {
